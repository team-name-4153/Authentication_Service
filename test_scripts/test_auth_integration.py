...
def test_register_endpoint(client):
    response = client.post('/register', json={'email': 'test_register@example.com', 'password': 'password123'})
...
def test_login_endpoint(client):
    client.post('/register', json={'email': 'test_login@example.com', 'password': 'password123'})
    response = client.post('/login', json={'identifier': 'test_login@example.com', 'password': 'password123'})
...
def test_verify_token_endpoint(client):
    client.post('/register', json={'email': 'test_token@example.com', 'password': 'password123'})
    login_response = client.post('/login', json={'identifier': 'test_token@example.com', 'password': 'password123'})
